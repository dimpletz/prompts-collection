...
| [markdown-viewer](plugins/markdown-viewer/) | `SessionStart` | At session start, checks whether `markdown-viewer-app` is installed and installs it via `pip` if Python is available. Checks for available updates and injects an upgrade notification context if a newer version exists. Notifies the user if Python is not available or installation fails. |
