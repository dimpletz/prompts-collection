...
| [current-date-injector](plugins/current-date-injector/) `v1.2.0` | Hook that injects the current date (YYYY-MM-DD) into the agent context at session start, and injects the command to determine the current time in 24-hr format with timezone. | — | — | `SessionStart`, `SubagentStart` |
...
| [current-date-injector](plugins/current-date-injector/) | `SessionStart`, `SubagentStart` | Injects today's date (`YYYY-MM-DD`) as `additionalContext` so agents always know the current date; also injects the command to determine the current time in 24-hr format with timezone (both hooks). |
